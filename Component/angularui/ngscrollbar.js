            ...
            compile: function (element) {
                ...
                return function (scope, element, attrs, ctrl) {
                    ...
                    var fun = function () {
                        ...
                        if ($element.parents(".center-panel") > 0) {
                            windowheight = ($element.parents(".center-panel").height() + $element.parents(".center-panel").offset().top);
                        }
                        var domHeight = 0;
                        if (scope.dottomelement) {
                            domHeight += $("#" + scope.dottomelement).height();
                        }
                        if (scope.position) {
                            domHeight += scope.position.bottom;
                        }
                        var Height = windowheight - $element.offset().top - domHeight;
                        $element.height(Height + "px");
                    }
                    fun();
                    //浏览器增加大小变换事件
                    $(window).unbind("resize", fun).bind("resize", fun);
                };
            }
        };
    }
...
.controller('scrollbarController', [function () {
    ...
    var defaultConfig = {
        ...
        scrollbottomX: function () {
            return true;
        },
        scrollbottomY: function () {
            return true;
        }
    };
        ...
        compile: function () {
            return function (scope, element, attrs, ctrl) {
                ...
                var scrollTo = function (top) {
                    //内容区域
                    var contentOffsetHeight = getContentHeight();
                    //框架区域
                    var containerOffsetHeight = getContainerHeight();

                    var gapHeight = containerOffsetHeight - contentOffsetHeight;
                    top = Math.min(0, Math.max(top, gapHeight));
                    //添加滚动到底触发事件
                    if (top < (gapHeight + 20) && typeof (config.scrollbottomX) == "function") {
                        config.scrollbottomX();
                    }
        ...
        compile: function () {
            return function (scope, element, attrs, ctrl) {
                ...
                var scrollTo = function (left) {
                    ...
                    //内容区域
                    var contentOffsetWidth = getContentWidth();
                    //框架区域
                    var containerOffsetWidth = getContainerWidth();

                    var gapWidth = containerOffsetWidth - contentOffsetWidth;

                    left = Math.min(0, Math.max(left, getContainerWidth() - getContentWidth()));
                    contentElement.css('margin-left', left + 'px');

                    //添加滚动到右触发事件
                    if (left < (gapWidth + 20) && typeof (config.scrollbottomY) == "function") {
                        config.scrollbottomY();
                    }
